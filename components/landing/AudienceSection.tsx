      ...
      <div className="page-container">
        ...
        <div className="mt-12 hidden gap-5 sm:grid sm:grid-cols-2 lg:grid-cols-3 md:mt-16">
          {AUDIENCES.map((audience, index) => {
            ...
            return <Reveal key={audience.title} delay={index * 0.05} className="h-full"><article className="group h-full rounded-3xl border border-slate-900/8 bg-[#f8fafc] p-6 transition-all duration-300 hover:-translate-y-1 hover:bg-white hover:shadow-soft"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-indigo-600 shadow-soft-sm"><Icon className="h-5 w-5" /></span><h3 className="mt-5 text-base font-semibold text-slate-900">{audience.title}</h3><p className="mt-2 text-sm leading-relaxed text-slate-500">{audience.description}</p></article></Reveal>;
          })}
        </div>
      </div>
    </section>
